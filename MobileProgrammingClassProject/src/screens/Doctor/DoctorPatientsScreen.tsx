...
export default function DoctorPatientsScreen({ navigation }: any) {
  ...
  // Carga inicial (obtener el doctor autenticado + su lista).
  useEffect(() => {
    const init = async () => {
  ...
  // 🔁 Fix de "lista stale": recargar cada vez que esta pestaña vuelve a
  // tener foco (por ejemplo, después de asignarte un paciente y navegar
  // fuera y volver).
  useFocusEffect(
    useCallback(() => {
      if (doctorId) {
        cargarMisPacientes(doctorId);
      }
    }, [doctorId, cargarMisPacientes])
  );
